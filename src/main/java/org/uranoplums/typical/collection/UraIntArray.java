/*
 * Copyright 2013-2015 the Uranoplums Foundation and the Others.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific language
 * governing permissions and limitations under the License.
 *
 * $Id: UraIntArray.java$
 */
package org.uranoplums.typical.collection;

import static org.uranoplums.typical.collection.factory.UraListFactory.*;

import java.lang.reflect.Array;
import java.util.Arrays;
import java.util.Collection;
import java.util.List;

/**
 * UraIntArrayクラス。<br>
 *
 * @since 2015/11/30
 * @author syany
 */
public class UraIntArray extends AbsUraArray {

    /**  */
    private static final long serialVersionUID = -7291461453180304810L;

    /**
     * 。<br>
     * @param array
     * @return
     */
    public static final List<Integer> asList(int... array) {
        UraIntArray res = new UraIntArray(array);
        return res.asList();
    }

    /**
     * 。<br>
     * @param list
     * @return
     */
    public static final int[] toArray(Collection<? extends Integer> list) {
        UraIntArray res = new UraIntArray(list);
        return res.getArray();
    }

    public UraIntArray() {
        super(int.class);
    }

    public UraIntArray(Collection<? extends Integer> list) {
        super(int.class, list.size());
        for (int i : list) {
            this.add(i);
        }
    }

    public UraIntArray(int capacity) {
        super(int.class, capacity);
    }

    public UraIntArray(int... array) {
        super(int.class, array.length);
        this.addAll(array);
    }

    public UraIntArray add(int value) {
        ensureCapacity(this.size + 1);
        int[] array = (int[]) this.elementData;
        array[(this.size++)] = value;
        return this;
    }

    public UraIntArray add(int index, int value) {
        if ((index > this.size) || (index < 0)) {
            throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + this.size);
        }
        ensureCapacity(this.size + 1);
        System.arraycopy(this.elementData, index, this.elementData, index + 1, this.size - index);
        int[] array = (int[]) this.elementData;
        array[index] = value;
        this.size += 1;
        return this;
    }

    /**
     * 。<br>
     * @param paramInt
     * @param array
     * @return
     */
    public boolean addAll(int paramInt, int... array) {
        if ((paramInt > this.size) || (paramInt < 0)) {
            throw new IndexOutOfBoundsException("Index: " + paramInt + ", Size: " + this.size);
        }
        int i = array.length;
        ensureCapacity(this.size + i);
        int j = this.size - paramInt;
        if (j > 0) {
            System.arraycopy(this.elementData, paramInt, this.elementData, paramInt + i, j);
        }
        System.arraycopy(array, 0, this.elementData, paramInt, i);
        this.size += i;
        return i != 0;
    }

    /**
     * 。<br>
     * @param array
     * @return
     */
    public boolean addAll(int... array) {
        int i = array.length;
        ensureCapacity(this.size + i);
        System.arraycopy(array, 0, this.elementData, this.size, i);
        this.size += i;
        return i != 0;
    }

    /**
     * 。<br>
     * @return
     */
    public List<Integer> asList() {
        List<Integer> res = newArrayList(this.size);
        for (int i : this.getArray()) {
            res.add(Integer.valueOf(i));
        }
        return res;
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.collection.AbsUraArray#clear()
     */
    @Override
    public UraIntArray clear() {
        return (UraIntArray) super.clear();
    }

    public boolean contains(int value) {
        return indexOf(value) >= 0;
    }

    public boolean containsAll(int... array) {
        boolean res = false;
        for (int i : array) {
            res = this.contains(i);
            if (!res) {
                break;
            }
        }
        return res;
    }

    public int get(int paramInt) {
        RangeCheck(paramInt);
        int[] array = (int[]) this.elementData;
        return array[paramInt];
    }

    public int[] getArray() {
        return (int[]) this.copyOfRange(0, this.size);
    }

    public int[] getArray(int start) {
        return (int[]) this.copyOfRange(start, this.size);
    }

    public int[] getArray(int start, int end) {
        return (int[]) this.copyOfRange(start, end + 1, this.size);
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.lang.UraObject#hashCode()
     */
    @Override
    public int hashCode() {
        return Arrays.hashCode(this.getArray());
    }

    public int indexOf(int value) {
        int j = 0;
        for (int i : this.getArray()) {
            if (i == value) {
                return j;
            }
            j++;
        }
        return -1;
    }

    public int lastIndexOf(int paramObject) {
        int rindex = this.size - 1;
        for (; rindex >= 0; rindex--) {
            if (this.get(rindex) == paramObject) {
                break;
            }
        }
        return rindex;
    }

    public boolean removeAll(int... array) {
        boolean res = false;
        for (int i : array) {
            if (res) {
                this.removeValue(i);
            } else {
                res = this.removeValue(i);
            }
        }
        this.resize();
        return res;
    }

    public int removeIndex(int paramInt) {
        RangeCheck(paramInt);
        int localObject = this.get(paramInt);
        this.fastRemove(paramInt);
        return localObject;
    }

    public boolean removeValue(int value) {
        boolean res = false;
        for (int i = this.indexOf(value); i >= 0; i = this.indexOf(value)) {
            fastRemove(i);
            res = true;
        }
        this.resize();
        return res;
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.collection.AbsUraArray#resetElement(int)
     */
    @Override
    protected void resetElement(int index) {
        Array.setInt(this.elementData, index, 0);
    }

    public boolean retainAll(int... array) {
        boolean res = false;
        UraIntArray del = new UraIntArray();
        int[] selfArray = (int[]) this.elementData;
        for (int i : selfArray) {
            boolean t = false;
            for (int j : array) {
                if (i == j) {
                    t = true;
                    break;
                }
            }
            if (!t) {
                del.add(i);
            }
        }
        if (!del.isEmpty()) {
            this.removeAll(del.getArray());
            res = true;
        }
        return res;
    }

    public int set(int paramInt, int paramE) {
        RangeCheck(paramInt);
        int[] array = (int[]) this.elementData;
        int localObject = array[paramInt];
        array[paramInt] = paramE;
        return localObject;
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.collection.AbsUraArray#sort()
     */
    @Override
    public UraIntArray sort() {
        int[] array = this.getArray();
        Arrays.sort(array);
        System.arraycopy(array, 0, this.elementData, 0, array.length);
        return this;
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.collection.AbsUraArray#sort(int, int)
     */
    @Override
    public UraIntArray sort(int start, int end) {
        int[] array = this.getArray();
        Arrays.sort(array, start, end + 1);
        System.arraycopy(array, 0, this.elementData, 0, array.length);
        return this;
    }

    /*
     * (非 Javadoc)
     *
     * @see org.uranoplums.typical.lang.UraObject#toString()
     */
    @Override
    public String toString() {
        return Arrays.toString(this.getArray());
    }
}
